...
// gylee start
 #define ADCVAL_X_MIN		120 //150 
 #define ADCVAL_X_MAX		3970 //3950 
 #define ADCVAL_Y_MIN		190 //280 
 #define ADCVAL_Y_MAX		3930 //3850 
...
//ylx add
			printk( 
				"DOWN point(%4d,%4d), pressure (%4u)\n",
				tc.x, tc.y, rt);
			if(tc.x > ADCVAL_X_MAX){
				tc.x = ADCVAL_X_MAX;
			}
			if(tc.y > ADCVAL_Y_MAX){
				tc.y = ADCVAL_Y_MAX;
			}
			...
			tc.x = LCD_SCREEN_X_PIXEL*(x)/(ADCVAL_X_MAX -ADCVAL_X_MIN); 
	 		tc.y = LCD_SCREEN_Y_PIXEL*(y)/(ADCVAL_Y_MAX-ADCVAL_Y_MIN);
			tc.x = LCD_SCREEN_X_PIXEL - tc.x;
//ylx add			//tc.y = LCD_SCREEN_Y_PIXEL - tc.y;
			tc.y = tc.y;
			printk( "return point(%4d,%4d), pressure (%4u)\n",
				tc.x, tc.y, rt);
